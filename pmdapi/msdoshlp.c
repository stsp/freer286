...
{
    if (!in_rsp16 || !reinit_entry.selector) {
	error("MSDOS: reinit is only supported from an RSP call\n");
	return;
    }
    if (host_reinit(_LWORD(eax), &reinit_entry, _ss, _esp - 0x100))
	error("MSDOS: the DPMI host refused to reinit\n");
}
...
/*
 * A program bound to Phar Lap's 286|DOS-Extender: an MZ loader, and right
 * after it the extender's image, which starts with "P2".
 */
static int is_pharlap286(const char *path)
{
    unsigned char h[6];
    unsigned long stub;
    int ret = 0;
    ...
    if (!f)
	return 0;
    if (fread(h, 1, 6, f) == 6 && h[0] == 'M' && h[1] == 'Z') {
	unsigned last = h[2] | (h[3] << 8);
	unsigned pages = h[4] | (h[5] << 8);

	stub = (pages - 1) * 512UL + (last ? last : 512);
	ret = pages && fseek(f, stub, SEEK_SET) == 0 &&
		fread(h, 1, 2, f) == 2 && h[0] == 'P' && h[1] == '2';
    }
    fclose(f);
    return ret;
}

/* the name the program was started with, after its environment */
static int psp_path(unsigned psp, char *buf, int len)
{
    unsigned env = READ_WORD(SEGOFF2LINEAR(psp, 0x2c));
    dosaddr_t p = SEGOFF2LINEAR(env, 0);
    int i;

    if (!env)
	return -1;
    while (READ_BYTE(p) || READ_BYTE(p + 1)) {
	if (p - SEGOFF2LINEAR(env, 0) > 0x8000)
	    return -1;
	p++;
    }
    p += 4;			/* the two zeroes and a word count */
    for (i = 0; i < len - 1 && READ_BYTE(p + i); i++)
	buf[i] = READ_BYTE(p + i);
    buf[i] = 0;
    return i ? 0 : -1;
}
...
static unsigned char takeover_stack[0x40000] __attribute__((aligned(16)));

static int takeover_main(void)
{
    int rc;
 ...
 * give it its exceptions on whatever stack it is on.
 */
static void takeover(void)
{
    call_on_stack(takeover_main, takeover_stack + sizeof(takeover_stack));
    error("MSDOS: %s did not terminate\n", takeover_path);
    for (;;);
}
...
{
    static const char sig[] = "286|DOS-Extender";
    dosaddr_t p = SEGOFF2LINEAR(psp, 0x100);
    unsigned mcb_paras = READ_WORD(SEGOFF2LINEAR(psp - 1, 3));
    dosaddr_t end = SEGOFF2LINEAR(psp, 0) + mcb_paras * 16;
    int i;

    if (end > p + 0x20000)
	end = p + 0x20000;
    for (; p + sizeof(sig) - 1 <= end; p++) {
	for (i = 0; sig[i] && READ_BYTE(p + i) == (unsigned char)sig[i]; i++);
	if (!sig[i])
	    return 1;
    }
    return 0;
...
/* the extender gives its program a PSP of its own, a child of the one it
 * was loaded with */
static int is_pharlap_client(unsigned psp)
{
    return has_extender(psp) ||
...
static void rsp_call(cpuctx_t *scp, int is_32)
{
	...
	hook_int31(clnt);
	if (takeover_ready) {
	    unsigned psp = _LWORD(esi);
	    unsigned parent = READ_WORD(SEGOFF2LINEAR(psp, 0x16));
	    if ((psp_path(psp, takeover_path, sizeof(takeover_path)) ||
		    !is_pharlap286(takeover_path)) &&
		    (psp_path(parent, takeover_path, sizeof(takeover_path)) ||
		    !is_pharlap286(takeover_path))) {
		error("MSDOS: the extender's program is not a bound one\n");
		takeover_path[0] = 0;
	    }
	    takeover_ready = 0;
	    takeover_pending = 1;
    ...
    /* the frame is the one the host made for the call */
    do_retf_x(scp, call_32);
}
...
void pmdapi_entry(struct entry_frame *f)
{
    ...
    } else if (takeover_pending && (id == ID_INT31 ||
	    (id >= ID_EXT0 && id < ID_EXT0 + MAX_EXT))) {
	/* the extender's first interrupt call after it entered DPMI: it
	 * gets no further, see takeover() */
	takeover_pending = 0;
	takeover();
    } else {
	_eip = stub_off(id);
