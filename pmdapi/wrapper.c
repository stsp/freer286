...
int dpmi_install_rsp(struct RSPcall_s *callback)
{
    ULONG base;
    uint32_t lin;
    unsigned char *d = callback->code16;

    if (__dpmi_get_descriptor(_my_ds(), callback->data16) == -1)
	return -1;
    memcpy(callback->data32, callback->data16, 8);
    /* a 16-bit client is called through a 16-bit code descriptor based on
     * rsp16_tramp in entry.S, which is where our 16-bit ip has to be */
    if (__dpmi_get_segment_base_address(_my_cs(), &base) == -1)
	return -1;
    rsp16_cs_base = base;
    lin = base + (uintptr_t)rsp16_tramp;
    d[0] = 0xff;
    d[1] = 0xff;
    d[2] = lin;
    d[3] = lin >> 8;
    d[4] = lin >> 16;
    d[5] = 0xfb;		/* present, DPL 3, readable code */
    d[6] = 0;			/* 16-bit, byte granular */
    d[7] = lin >> 24;
    callback->ip = 0;
